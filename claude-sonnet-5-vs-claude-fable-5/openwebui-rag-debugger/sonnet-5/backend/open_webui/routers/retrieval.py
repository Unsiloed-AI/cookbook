...
####################################
#
# RAG Debugger (Chunk Inspector)
#
# Read-only introspection over the existing RAG pipeline: lets a user see a
# document's processing status, extracted text, and stored chunks, and run a
# real test query against that document's own chunks using the same
# embedding function / vector search primitive normal chat retrieval uses.
#
####################################


def _resolve_file_collection_name(file: FileModel) -> str:
    """The vector collection currently holding this file's chunks.

    Standalone uploads live in a private `file-{id}` collection; once a file
    is added to a Knowledge base, its chunks are (re-)embedded into that KB's
    own collection and `file.meta['collection_name']` is updated to point at
    it (see `process_file` / `Files.update_file_metadata_by_id` above).
    """
    return (file.meta or {}).get('collection_name') or f'file-{file.id}'


async def _get_file_for_debug(file_id: str, user) -> FileModel:
...
DEBUG_TEXT_PREVIEW_LIMIT = 2000
DEBUG_CHUNK_LIMIT = 500


@router.get('/debug/{file_id}')
async def get_file_debug_info(
    file_id: str,
    request: Request,
    user=Depends(get_verified_user),
):
    file = await _get_file_for_debug(file_id, user)
    collection_name = _resolve_file_collection_name(file)
    ...
    response = {
        ...
        'status': file_status,
        'collection_name': collection_name,
        'collection_scope': 'file' if collection_name == f'file-{file.id}' else 'knowledge',
    }

    if file_status == 'failed' and data.get('error'):
        response['error'] = data.get('error')

    if text_content:
        response['extracted_text_length'] = len(text_content)
        response['extracted_text_truncated'] = len(text_content) > DEBUG_TEXT_PREVIEW_LIMIT
        response['extracted_text_preview'] = text_content[:DEBUG_TEXT_PREVIEW_LIMIT]

    chunks = []
    chunks_truncated = False
    if await asyncio.to_thread(VECTOR_DB_CLIENT.has_collection, collection_name):
        result = await ASYNC_VECTOR_DB_CLIENT.query(
            collection_name=collection_name,
            filter={'file_id': file.id},
            limit=DEBUG_CHUNK_LIMIT,
        )
        if result is not None and result.ids and len(result.ids[0]) > 0:
            ids = result.ids[0]
            documents = result.documents[0] if result.documents else []
            metadatas = result.metadatas[0] if result.metadatas else []
            chunks_truncated = len(ids) >= DEBUG_CHUNK_LIMIT
    ...
    response['chunks_truncated'] = chunks_truncated
    response['chunks'] = chunks

    return response


class FileDebugQueryForm(BaseModel):
    query: str
    k: int | None = None


@router.post('/debug/{file_id}/query')
async def query_file_debug(
    file_id: str,
    form_data: FileDebugQueryForm,
    request: Request,
    user=Depends(get_verified_user),
):
    file = await _get_file_for_debug(file_id, user)
    ...
    if file_status != 'completed':
        message = (
            f'Document processing failed: {data.get("error")}'
            if file_status == 'failed'
            else 'Document is still processing — retrieval is not available yet.'
        )
        return {**base_response, 'k': None, 'results': [], 'message': message}

    if not await asyncio.to_thread(VECTOR_DB_CLIENT.has_collection, collection_name):
        return {
            **base_response,
            'k': None,
            'results': [],
            'message': 'No vector data found for this document.',
        }
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=ERROR_MESSAGES.DEFAULT(e, 'Error running debug retrieval query'),
        )
    ...
    if result is not None and result.ids and len(result.ids[0]) > 0:
        ...
        documents = result.documents[0] if result.documents else []
        metadatas = result.metadatas[0] if result.metadatas else []
        distances = result.distances[0] if getattr(result, 'distances', None) else None
        ...
        for idx, chunk_id in enumerate(ids):
            ...
            if distances is not None and idx < len(distances):
                hit['score'] = distances[idx]

            source = metadata.get('source') or metadata.get('name')
            if source:
                hit['source'] = source
            if metadata.get('file_id'):
                hit['file_id'] = metadata.get('file_id')
            if metadata.get('page') is not None:
                hit['page'] = metadata.get('page')

            results.append(hit)

    message = None if results else 'No results returned for this query.'

    return {**base_response, 'k': k, 'results': results, 'message': message}
