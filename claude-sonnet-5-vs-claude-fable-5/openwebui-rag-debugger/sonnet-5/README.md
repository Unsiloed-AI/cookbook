...
- 🔬 **RAG Debugger**: Inspect what your RAG pipeline actually retrieves before it reaches the model — processing status, extracted text, generated chunks, and a real test-query tool, right from the Knowledge base file view. See [RAG Debugger](#rag-debugger-) below.
...
Chat only shows you the final answer — the RAG Debugger shows you the hidden retrieval layer behind it, using your real data and the same retrieval code path chat uses (no mock/sample data).

- **What it does**: Lets you inspect a Knowledge base document's processing status, its extracted text, and its generated chunks, and run a real test query to see exactly which chunks would be retrieved for it — before any LLM answer is generated.
- **Where to find it**: Open a Knowledge base, click a document to open its detail panel, then click **Inspect Retrieval**.
- **How to use it**: Check the processing status and extracted text/chunks, then type a question into the test query box and click **Run Retrieval**.
- **What the results mean**: Each retrieved chunk shows its rank and, where available, a similarity score, source filename, chunk/document id, page number, and metadata. Fields that aren't available for your setup (e.g. `page` for non-paginated files) are simply omitted rather than faked.
- **Known limitations**: The chunk list is capped at 500 chunks per document. The test query is scoped to the selected document's own chunks and always runs a plain vector similarity search — it does not exercise hybrid (BM25) search or reranking even if enabled in your RAG settings, so ranking/scores may differ slightly from live chat retrieval on such setups.
