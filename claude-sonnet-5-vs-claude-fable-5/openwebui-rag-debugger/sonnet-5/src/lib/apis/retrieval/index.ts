...
export const getFileDebugInfo = async (token: string, fileId: string) => {
	let error = null;

	const res = await fetch(`${RETRIEVAL_API_BASE_URL}/debug/${fileId}`, {
		method: 'GET',
		headers: {
			Accept: 'application/json',
			'Content-Type': 'application/json',
...
export const queryFileDebug = async (
	token: string,
	fileId: string,
	query: string,
	k: number | null = null
	...
	const res = await fetch(`${RETRIEVAL_API_BASE_URL}/debug/${fileId}/query`, {
		method: 'POST',
		headers: {
			Accept: 'application/json',
			'Content-Type': 'application/json',
