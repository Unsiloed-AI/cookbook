<script lang="ts">
	...
	import RagDebuggerModal from './KnowledgeBase/RagDebuggerModal.svelte';
			...
			{:else}
				...
				{#if fileItems !== null && fileItemsTotal !== null}
					<div class="flex flex-row flex-1 gap-3 px-2.5 mt-2">
								...
								<div class="flex flex-col justify-start h-full max-h-full">
									<div class=" flex flex-col w-full h-full max-h-full">
										<div class="shrink-0 flex items-center p-2">
											...
											<div>
												<button
													class="flex self-center w-fit text-sm py-1 px-2.5 dark:text-gray-300 dark:hover:text-white hover:bg-black/5 dark:hover:bg-white/5 rounded-lg gap-1.5 items-center"
													type="button"
													on:click={() => {
														showRagDebugger = true;
													}}
												>
													<Search className="size-3.5" />
													{$i18n.t('Inspect Retrieval')}
												</button>
											</div>
						...
						<RagDebuggerModal
							bind:show={showRagDebugger}
							fileId={selectedFileId}
							fileName={selectedFile?.meta?.name}
							onClose={() => {
								showRagDebugger = false;
							}}
						/>
					</div>
				{:else}
