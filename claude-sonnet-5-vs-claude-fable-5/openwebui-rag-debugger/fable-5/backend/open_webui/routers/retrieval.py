...
DEBUG_MAX_CHUNKS = 100
DEBUG_MAX_CONTENT_PREVIEW = 10000
DEBUG_MAX_CHUNK_TEXT = 4000
...
async def get_file_debug_info(
    ...
    """Debug info for the RAG Debugger: processing status, extracted text
    preview and the chunks stored in the vector DB for a file."""
    file = await Files.get_file_by_id(file_id, db=db)
    if not file:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=ERROR_MESSAGES.NOT_FOUND)

    if not (file.user_id == user.id or user.role == 'admin' or await has_access_to_file(file_id, 'read', user, db=db)):
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=ERROR_MESSAGES.NOT_FOUND)

    config = await get_retrieval_config()
    bypass = bool(config.BYPASS_EMBEDDING_AND_RETRIEVAL)

    data = file.data or {}
    meta = file.meta or {}
    content = data.get('content') or ''
    # Legacy files processed before the status field existed have content
    # but no status; treat them as completed so their chunks are shown.
    file_status = data.get('status') or ('completed' if content else 'pending')
    content_info = {
        'preview': content[:DEBUG_MAX_CONTENT_PREVIEW],
        'length': len(content),
        'truncated': len(content) > DEBUG_MAX_CONTENT_PREVIEW,
    }
    ...
    if file_status == 'completed' and not bypass:
        # The file's own collection is created on upload; files linked to a
        # knowledge base may only have chunks in the KB collection (which
        # meta.collection_name then points to), so try both.
        candidates = [f'file-{file_id}']
        if meta.get('collection_name') and meta['collection_name'] not in candidates:
            candidates.append(meta['collection_name'])
        ...
        try:
            for collection_name in candidates:
                ...
                result = await ASYNC_VECTOR_DB_CLIENT.query(
                    ...
                    limit=DEBUG_MAX_CHUNKS + 1,
                )
                if not (result and result.ids and result.ids[0]):
                    continue
                ...
                items = [
                    {
                        'id': ids[idx],
                        'text': (documents[idx] if idx < len(documents) else '') or '',
                        'metadata': (metadatas[idx] if idx < len(metadatas) else None) or {},
                    }
                    for idx in range(len(ids))
                ]
                # Vector DB query order is arbitrary; restore document order.
                items.sort(
                    key=lambda item: (
                        not isinstance(item['metadata'].get('start_index'), (int, float)),
                        item['metadata'].get('start_index') or 0,
                    )
                )

                chunks['collection_name'] = collection_name
                chunks['truncated'] = len(items) > DEBUG_MAX_CHUNKS
                items = items[:DEBUG_MAX_CHUNKS]
                for item in items:
                    item['text'] = item['text'][:DEBUG_MAX_CHUNK_TEXT]
                chunks['items'] = items
                chunks['count'] = len(items)
                break
        except Exception as e:
            log.exception(f'Failed to fetch chunks for file {file_id}')
            chunks['error'] = str(e)

    return {
        'file': {
            'id': file.id,
            'name': meta.get('name', file.filename),
            'status': file_status,
