...
- 🔬 **RAG Debugger (Chunk Inspector)**: Inspect what the RAG layer actually retrieves before any model answers. In **Workspace → Knowledge**, open a knowledge base, click the `⋯` menu on a file, and choose **Inspect RAG** to see the file's processing status, extracted text preview, and stored chunks, and to run a test query through the same retrieval pipeline chat uses. Retrieved chunks are shown ranked, with relevance scores where available (higher is better; scores come from the reranker when hybrid search is enabled). Limitations: the chunk list is capped at 100 chunks, queries run against the whole knowledge base collection (so results can include other files, highlighted accordingly), and no chunks are available when embedding/retrieval bypass is enabled.
