<script lang="ts">
	...
	import RAGDebugModal from './KnowledgeBase/RAGDebugModal.svelte';
	...
	let showRAGDebugModal = false;
	let debugFileId = null;
	let debugFileName = '';
									...
									{#if fileItems.length > 0 || directoryItems.length > 0}
										<div class=" flex overflow-y-auto h-full w-full scrollbar-hidden text-xs">
											<Files
												...
												onDebug={(fileId) => {
													const file = fileItems?.find((file) => file.id === fileId);
													debugFileId = fileId;
													debugFileName = file?.name ?? file?.meta?.name ?? '';
													showRAGDebugModal = true;
												}}
												onDelete={(fileId) => {
													selectedFileId = null;
													selectedFile = null;
